...
const forgetPassword = async (req, res, { userModel }) => {
  ...
  const finrarmag_app_email = settings['finrarmag_app_email'];
  const finrarmag_base_url = settings['finrarmag_base_url'];

  const url = checkAndCorrectURL(finrarmag_base_url);
  ...
  await sendMail({
    ...
    name: user.name,
    link,
    subject: 'Reset your password | finrarmag',
    finrarmag_app_email,
