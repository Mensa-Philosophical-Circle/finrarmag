...
const setup = async (req, res) => {
  ...
  for (const filePath of settingsFiles) {
    ...
    const settingsToUpdate = {
      finrarmag_app_email: email,
      finrarmag_app_company_email: email,
      finrarmag_app_timezone: timezone,
      finrarmag_app_country: country,
      finrarmag_app_language: language || 'en_us',
    };
  ...
  return res.status(200).json({
    success: true,
    result: {},
    message: 'Successfully finrarmag App Setup',
  });
};
